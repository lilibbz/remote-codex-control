...
它的核心场景是：你正在电脑上让 Codex 处理一个工程项目，突然需要出门，但还想用手机继续指挥当前这个 Codex 工程线程。手机负责提交指令，电脑端 Codex 线程负责读取、执行，在电脑端正常输出完整回复，并把简短结果写回手机页面。
...
手机页面上的结果回执只是摘要，不应该替代电脑端 Codex 的正常回复。
...
注意：启动桥接服务只代表手机可以提交指令。要让当前 Codex App 线程读取这些指令，还需要设置手动检查或 heartbeat 检查间隔。推荐默认值是每 15 分钟检查一次，并设置明确的持续时间。
...
```text
接下来 4 小时内，每 15 分钟使用 $remote-codex-control 检查一次手机指令收件箱。如果有待处理指令，就在当前 Codex App 线程中执行，在电脑端正常输出完整回复，并写入简短的手机可见结果说明。
```
...
当 Codex 在当前线程里处理手机提交的指令时，电脑端 Codex App 仍然应该显示正常回复：真正的答案、计划、实现总结、发现的问题或执行结果。手机页面只显示一个简短状态摘要。

例如，如果手机指令是“帮我评审这个项目”，电脑端应该输出实际评审内容；手机端可以只显示：

```text
已完成：我评审了项目，并把主要优化建议写在电脑端 Codex 线程里。
```

不要让电脑端只输出“已处理并写回手机”。这句话适合做手机摘要，但不适合作为电脑端回复。
